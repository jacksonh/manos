...
namespace Mango.Templates.Minge {
	...
	public class ConstantDoubleValue : ConstantValue {
		...
		public override void Emit (Application app, Page page, CilWorker worker)
		{
			...
			Console.WriteLine ("CREATING A CONSTANT DOUBLE VALUE:  {0}  {1}", Value, ResolvedType);
		}
	}
	...
	public class Application {
		...
		public Application (Environment environment, string name, string path)
		{
			Name = name;
			Path = path;
		...
		public Page CreatePage (string path)
		{
			...
			TypeDefinition page = new TypeDefinition (name, ns.ToString (), TypeAttributes.Public | TypeAttributes.BeforeFieldInit,
	...
	public class Page {
		...
		private void EmitBaseRenderToStreamCall ()
		{
			...
			worker.InsertAfter (first_instruction, worker.Create (OpCodes.Callvirt, base_render));
		...
		private Dictionary<string,List<Instruction>> block_calls = new Dictionary<string,List<Instruction>> ();
		...
		public void BeginBlock (string name)
		{
			...
			AddToBlockCallList (name, block_call);
			UpdateBlockCallsInParent (name, meth);

			method_stack.Push (meth);
		}

		private void AddToBlockCallList (string name, Instruction block_call)
		{
			List<Instruction> call_list = null;

			if (!block_calls.TryGetValue (name, out call_list)) {
				call_list = new List<Instruction> ();
				block_calls [name] = call_list;
			}

			call_list.Add (block_call);
		}

		private void UpdateBlockCallsInParent (string name, MethodReference meth)
		{
			if (base_type == null)
				return;

			base_type.UpdateBlockCalls (name, meth);
		}

		private void UpdateBlockCalls (string name, MethodReference meth)
		{
			List<Instruction> call_list = block_calls [name];

			foreach (Instruction call in call_list) {
				call.Operand = meth;
			}
		...
		public void EmitToString (Application app, Page page, CilWorker worker, TypeReference resolved)
		{
			...
			Instruction inst = worker.Emit (OpCodes.Call, (MethodReference) method);
		}
		...
		{
			Console.WriteLine ("beginning macro: {0} ({1})", name, args.Count);
		...
		public MethodDefinition AddRenderMethod (string name, List<ArgumentDefinition> extra_args=null)
		{
			...
			MethodDefinition render = new MethodDefinition (name, MethodAttributes.Public, assembly.MainModule.Import (typeof (void)));
			...
			if (extra_args != null) {
				...
				foreach (ArgumentDefinition arg in extra_args) {
					...
					if (arg.DefaultValue != null) {
						Console.WriteLine ("ADDING CONSTANT DEFINITION:  {0}", arg.DefaultValue.GetValue ());
