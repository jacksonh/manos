...
run-tests: all
	$(NUNIT_CONSOLE) build/manos/Manos.Tests.dll

update-docs: all
	mdoc update -o ./docs/api/en ./build/manos/Manos.dll
...
install-bin: all
	...
	cp -rf ./build/manos/* $(install_bin_dir)
...
gem: all
	cp release/manos.gemspec build/manos/.
